...
public class PhoneService implements IPhoneService {
    ...
    public List<Phone> getAllPremiumPhones() {
        return phoneRepository.findAllPhones().stream()
                .filter(phone -> phone.getCost() >= 400 && !phone.getType().equals(PhoneType.BASIC))
                .collect(Collectors.toList());
    }

    public List<Phone> getSamsungPhones() {
        return phoneRepository.findAllPhones().stream()
                .filter(phone -> phone.getBrand().equals(PhoneBrand.SAMSUNG))
                .collect(Collectors.toList());
    }

    public List<Phone> getPremiumSamsungPhones() {
        return phoneRepository.findAllPhones().stream()
                .filter(phone -> phone.getBrand().equals(PhoneBrand.SAMSUNG) && phone.getCost() >= 400
                        && !phone.getType().equals(PhoneType.BASIC))
                .collect(Collectors.toList());
    }

    public List<Phone> getSamsungAndHTCPhones() {
        return phoneRepository.findAllPhones().stream()
                .filter(phone -> phone.getBrand().equals(PhoneBrand.SAMSUNG) || phone.getBrand().equals(PhoneBrand.HTC))
                .collect(Collectors.toList());
    }
    ...
    public List<Phone> getPremiumSamsungAndHTCPhones() {
        return phoneRepository.findAllPhones().stream()
                .filter(phone -> (phone.getBrand().equals(PhoneBrand.SAMSUNG)
                        || phone.getBrand().equals(PhoneBrand.HTC))
                        && phone.getCost() >= 400 && !phone.getType().equals(PhoneType.BASIC))
                .collect(Collectors.toList());
    }
    ...
    public List<Phone> getAllPremiumExceptSamsungAndHTCPhones() {
        return phoneRepository.findAllPhones().stream()
                .filter(phone -> !phone.getBrand().equals(PhoneBrand.SAMSUNG)
                        && !phone.getBrand().equals(PhoneBrand.HTC) && phone.getCost() >= 400
                        && !phone.getType().equals(PhoneType.BASIC))
                .collect(Collectors.toList());
    }
